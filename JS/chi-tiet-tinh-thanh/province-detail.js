document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch(
    `http://localhost:5151/api/Provinces?search=${encodeURIComponent(
      provinceName
    )}`
  )
    .then((res) => res.json())
    .then((provinces) => {
      if (!provinces.length) {
        document.getElementById("province-specialties").innerHTML =
          "<p>Không tìm thấy tỉnh này!</p>";
        return;
      }
      const provinceId = provinces[0].id;
      // Lấy đặc sản theo provinceId
      return fetch(
        `http://localhost:5151/api/Specialties?provinceId=${provinceId}`
      );
    })
