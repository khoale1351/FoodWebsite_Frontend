document.addEventListener('DOMContentLoaded', () => {
    ...
    if (specialtyId) {
        fetchAPI(`/Specialties/${specialtyId}`, {}, false)
            .then(data => {
                const detail = document.getElementById('specialty-detail');
                detail.innerHTML = `
                    <h2 class="text-2xl font-bold">${data.name}</h2>
                    <img src="${getValidImageUrl(data.specialtyImages?.[0]?.imageUrl)}" alt="${data.name}" class="w-full h-64 object-cover rounded">
                ...
                const token = localStorage.getItem('token');
                if (token) {
                    fetchAPI('/UserViewHistory', {
                        method: 'POST',
                        body: {
                            userId: 'current_user', // Giả định lấy từ profile
