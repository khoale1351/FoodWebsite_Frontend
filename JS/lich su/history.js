    ...
    if (token) {
        ...
        fetch('http://localhost:5151/api/Auth/profile', {
            ...
            .then(user => {
                ...
                if (usernameDisplay) usernameDisplay.textContent = user.fullName || user.email || 'Người dùng';
            ...
            .then(data => {
                document.getElementById('history-list').innerHTML = data.length
                    ? data.map(item => `<div>${item.specialtyName || item.recipeName || 'Không rõ'}</div>`).join('')
