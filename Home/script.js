...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("http://localhost:5151/api/Provinces", {
    headers: {
      ...
      document
        ...
        .addEventListener("submit", function (e) {
          ...
          const query = document
            ...
            .value.toLowerCase();
          const results = specialties.filter((item) =>
  ...
  fetch("http://localhost:5151/api/Specialties", {
    ...
    .catch((error) => {
      ...
      document.getElementById("top-searches-list").innerHTML =
        "<li>Lỗi khi tải dữ liệu</li>";
    });
    ...
    .then((data) => {
      ...
      if (region) {
        filteredData = filteredData.filter((item) => {
          return fetch(`http://localhost:5151/api/Provinces/${item.Id}`, {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token") || ""}`,
            },
          })
